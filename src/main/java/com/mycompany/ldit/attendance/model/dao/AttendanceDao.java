...
public class AttendanceDao {
	...
	public int countXAList(Map<String, Object> mapS) {
		return sqlSession.selectOne("attCheck.countXAList", mapS);
	}
	public int countWhomeList(Map<String, Object> mapS) {
		return sqlSession.selectOne("attCheck.countWhomeList", mapS);
	}
	...
	public List<WorkHomeApply> getWhomeList(int stfNo, int currentPage, int limitInOnePage, String keyValue) {
		...
		RowBounds row = new RowBounds(startRow, limitInOnePage);
		if (keyValue.equals(onlyAble)) {
			return sqlSession.selectList("attCheck.getWhomeListOnly", stfNo, row);
		} else {
			return sqlSession.selectList("attCheck.getWhomeList", stfNo, row);
		}
	}
	...
	public List<Attendance> getAttList(int stfNo, int currentPage, int limitInOnePage) {
		int startRow = (currentPage - 1) * limitInOnePage;
		RowBounds row = new RowBounds(startRow, limitInOnePage);
		return sqlSession.selectList("attCheck.getAttList", stfNo, row);
	}

	public int insertXiuxiApply(Map<String, Object> mapM) {
		return sqlSession.insert("aboutAtt.insertXiuxiApply", mapM);
	}

	public int insertWorkHomeApply(Map<String, Object> mapM) {
		return sqlSession.insert("aboutAtt.insertWorkHomeApply", mapM);
	}
	
	public int countAttList(int stfNo) {
		return sqlSession.selectOne("attCheck.countAttList", stfNo);
	}
	
	public String getRestAll(Map<String, Object> mapS) {
		return sqlSession.selectOne("aboutAtt.getRestAll", mapS);
	}
	
	public int updateRestAll(Map<String, Object> mapS) {
		return sqlSession.update("aboutAtt.updateRestAll", mapS);
	}
}
